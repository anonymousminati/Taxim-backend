    ...
    buildContextualPrompt(userPrompt, sessionId, isRetry = false) {
        ...
        if (context.conversationHistory.length > 0) {
            ...
            context.conversationHistory.slice(-2).forEach(entry => {
                contextualPrompt += `\n${entry.type}: ${entry.content.substring(0, 150)}${entry.content.length > 150 ? '...' : ''}`;
            });
        }
        ...
        if (context.previousCodes.length > 0 && !isRetry) {
            ...
            if (successfulCodes.length > 0) {
                ...
                successfulCodes.slice(-2).forEach((codeInfo, index) => {
                    contextualPrompt += `\n${index + 1}. Previous code used: ${codeInfo.code.substring(0, 200)}...`;
                });
        ...
        try {
            ...
            console.log('Prompt preview:', contextualPrompt.substring(0, 300) + '...');
    ...
    async savePythonFile(code, filename = 'animation.py') {
        ...
        while (attempts < maxRetries) {
            try {
                ...
                const className = classMatch ? classMatch[1] : 'Animation';                // Clean media directory before rendering to avoid confusion
                ...
                if (fs.existsSync(mediaDir)) {
                    ...
                    foundVideoPath = this.findVideoInMediaDir(mediaDir, className, baseFileName);
                }
